...
class PangramTest extends AnyFunSuite with Matchers {
  ...
  test("pangram with only lower case") {
    Pangrams.isPangram("the quick brown fox jumps over the lazy dog") should be(
      true
    )
  }
  ...
  test("another missing character, e.g. 'h'") {
    Pangrams.isPangram("five boxing wizards jump quickly at it") should be(
      false
    )
  }

  test("pangram with underscores") {
    Pangrams.isPangram("the_quick_brown_fox_jumps_over_the_lazy_dog") should be(
      true
    )
  }
  ...
  test("missing letters replaced by numbers") {
    Pangrams.isPangram("7h3 qu1ck brown fox jumps ov3r 7h3 lazy dog") should be(
      false
    )
  }
  ...
  test(
    ...
    Pangrams.isPangram("the quick brown fox jumps over with lazy FX") should be(
      false
    )
  }
}
